"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const locale = {
    "mailbox": {
        "title": {
            "inbox": "Inbox",
            "sent": "Gipadala",
            "drafts": "Drafts",
            "trash": "Basur",
            "junk": "Spam"
        },
        "delete": "Pagwagtang folder",
        "empty": "Wala’y sulod ang kini nga mailbox"
    },
    "message": {
        "labels": {
            "from": "Gikan:",
            "to": "Sa:",
            "date": "Gipadala:"
        }
    },
    "mailboxMessage": {
        "to": "Sa:"
    },
    "login": {
        "title": "Pag-sign up",
        "action": "Pag-sign up",
        "labels": {
            "username": "Username",
            "password": "Password"
        }
    },
    "accountButton": {
        "logout": "Pag-sign out",
        "myAccount": "Akong account",
        "filters": "Mga filter"
    },
    "compose": {
        "labels": {
            "to": "Sa:",
            "subject": "Pinili:",
            "cc": "Cc:",
            "bcc": "Bcc:"
        },
        "tabs": {
            "newMessageTitle": "Bag-ong mensahe"
        }
    },
    "editor": {
        "cmd": {
            "undo": "Pagtangtang",
            "redo": "Redo",
            "fontName": "Matang sa font",
            "fontSize": "Gidak-on sa font",
            "bold": "Buang",
            "italic": "Italiko",
            "underline": "Pag-underline",
            "justifyLeft": "Pahanay sa wala",
            "justifyCenter": "Pag-ilis sa tunga",
            "justifyRight": "Pag-ilis sa tuo",
            "insertUnorderedList": "Paglista",
            "insertOrderedList": "Lista sa nerbiyos",
            "removeFormat": "Kuhaa ang format"
        },
        "color": {
            "tooltip": "Kolor",
            "foreColor": "Text",
            "backColor": "Kasaysayan"
        },
        "upload": {
            "tooltip": "Paglakip",
            "add": "Pagdugang",
            "remove": "Kuhaa"
        },
        "send": "Gipadala"
    },
    "selection": {
        "title": [
            "{n} nga mga mensahe",
            "{n} nga mensahe",
            "{n} nga mga mensahe"
        ]
    },
    "actions": {
        "backToMailbox": "Balik sa mailbox",
        "reload": "Refresh",
        "select": "Pagpili",
        "markAsUnread": "Markahan ingon nga dili mabasa",
        "markAsRead": "Markahi ingon magbasa",
        "moveTo": "Balhin sa",
        "delete": "Pagwagtang",
        "deletePermanently": "Pagwagtang sa permanente",
        "discardDrafts": "Pagtangtang mga draft",
        "markAsSpam": "Markahi isip spam",
        "unMarkAsSpam": "Dili kini spam",
        "forward": "Sa unahan",
        "reply": "Tubag",
        "attachments": "Mga gilakip"
    },
    "weekDays": {
        "0": "Domingo",
        "1": "Lunes",
        "2": "Martes",
        "3": "Miyerkules",
        "4": "Huwebes",
        "5": "Biyernes",
        "6": "Sabado"
    },
    "months": {
        "0": "Enero",
        "1": "Pebrero",
        "2": "Marso",
        "3": "Abril",
        "4": "Mayo",
        "5": "Jun",
        "6": "Hulyo",
        "7": "Agosto",
        "8": "Septyembre",
        "9": "Oktubre",
        "10": "Nobyembre",
        "11": "Disyembre"
    },
    "notifier": {
        "messageSent": "Gipadala ang mensahe",
        "mailboxDeleted": "Ang Folder natangtang"
    },
    "drawerActions": {
        "createMailbox": {
            "label": "Bag-ong folder",
            "success": "Gibuhat ang folder"
        }
    },
    "dialogs": {
        "createMailbox": {
            "title": "Paghimo bag-ong folder",
            "label": "Ngalan sa folder",
            "accept": "Paghimo",
            "cancel": "Pagkansela"
        },
        "deleteMailbox": {
            "title": "Pagwagtang folder nga \"{mailbox}\"",
            "desc": "Pag-amping. Kini nga aksyon magtangtang sa tanan nga mga mensahe sa folder",
            "accept": "Pagwagtang",
            "cancel": "Pagkansela"
        }
    },
    "myAccount": {
        "title": "Akong account",
        "commonActions": {
            "title": "Kasagaran nga mga lihok",
            "updatePassword": "Pagbag-o sa password",
            "currentPassword": "Karon nga password",
            "newPassword": "Bag-ong password",
            "confirmPassword": "Gikumpirma ang bag-ong password"
        },
        "limits": {
            "gbUsed": "{gb} GB",
            "gbTotal": "sa {gb} GB",
            "messagesUsed": [
                "{n} nga mga mensahe",
                "{n} nga mensahe",
                "{n} nga mga mensahe"
            ],
            "messagesTotal": [
                "sa {n} nga mga mensahe",
                "sa {n} nga mensahe",
                "sa {n} nga mga mensahe"
            ],
            "storage": {
                "title": "Pagtipig"
            },
            "imapDownload": {
                "title": "IMAP Pag-download",
                "comment": "adlaw-adlaw"
            },
            "imapUpload": {
                "title": "Pag-upload sa IMAP",
                "comment": "adlaw-adlaw"
            },
            "pop3Download": {
                "title": "Pag-download sa POP3",
                "comment": "adlaw-adlaw"
            },
            "received": {
                "title": "Gidawat",
                "comment": "sa minuto"
            },
            "recipients": {
                "title": "Gipadala",
                "comment": "adlaw-adlaw"
            },
            "forwards": {
                "title": "Gibalikbalik",
                "comment": "adlaw-adlaw"
            }
        }
    },
    "filters": {
        "title": "Mga filter",
        "commingSoon": "Malapit na"
    }
};
exports.default = locale;
